...
Invoke-WebRequest "http://download.knime.org/analytics-platform/win/$KNIME_FILE" -outfile "$KNIME_FILE" 
unzip -q "$KNIME_FILE"

$WF_ZIP="wf.zip"
Invoke-WebRequest https://dl.bintray.com/silebat/build_pipeline_test_wf/$WF_ZIP -outfile $WF_ZIP
unzip $WF_ZIP -d wf
rm $WF_ZIP
